...
export function ConfigurablesCSSBridge() {
  ...
  useEffect(() => {
    ...
    if (isValidColor(brandColor.primary)) {
      ...
      // Thread app tokens
      root.style.setProperty("--th-primary", brandColor.primary);
    }

    // ── Secondary ────────────────────────────────────────────────────────
    if (isValidColor(brandColor.secondary)) {
      root.style.setProperty("--secondary", brandColor.secondary);
      root.style.setProperty("--th-secondary", brandColor.secondary);
    }
    ...
    if (isValidColor(brandColor.accent)) {
      ...
      root.style.setProperty("--th-accent", brandColor.accent);
    }
  }, [config?.brandColor]);
